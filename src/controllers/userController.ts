...
const registerUser = async ({ body }: Request, res: Response) => {
  ...
  try {
    ...
    await emailRegister(email, name, lastname, token);
...
const updateUserPassword = async ({ params, body, user }: UserResquestProvider, res: Response) => {
  ...
  try {
    ...
    if (userProfile._id.toString() !== user!._id.toString()) return res.status(401).json({ ok: false, msg: 'No tiene permisos para realizar esta acción' });
